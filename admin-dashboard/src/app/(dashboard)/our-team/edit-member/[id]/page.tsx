// app/(dashboard)/our-team/edit-member/[id]/page.tsx
"use client"
...
export default function EditTeamMemberPage() {
  ...
  const { data: member, isLoading: isFetching, isError } = useGetSingleTeamMemberQuery(id)
  const [updateTeamMember, { isLoading: isSaving }] = useUpdateTeamMemberMutation()
  ...
  if (isError || !member) {
    return (
      <div className="rounded-xl border border-red-200 bg-red-50 px-6 py-10 text-center">
        <p className="font-medium text-red-700">Team member not found</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="container mx-auto space-y-6">
      ...
      <div>
        ...
        <h1 className="mt-1 text-2xl font-bold text-slate-900">Edit Team Member</h1>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1 space-y-2">
            <label className="text-sm font-medium text-slate-700">Replace Photo (optional)</label>
            <input
              type="file"
              accept="image/*"
